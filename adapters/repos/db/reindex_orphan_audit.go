...
// cluster upgrading into it brings one per completed-and-unpromoted migration,
// and a sweep that is not told about it removes it while the tracker that owns
// it still stands.
...
func migrationSidecarDirsIn(dirName, prefix string, generation int, properties []string, withBackup bool) []string {
	...
	for _, propName := range properties {
		...
		if withBackup && suffixes.legacyBackupSuffix != "" {
			out = append(out, main+suffixes.legacyBackupSuffix+genTail)
		}
		if reindexSuffix != "" {
